...
namespace SupportTuoiBeo.Excel
{
    ...
    {
        public static void Export<T>(string path, List<T> entities)
        {
            using var spreadsheet = SpreadsheetDocument.Create(path, SpreadsheetDocumentType.Workbook);
            CreatePartsForExcel(spreadsheet, entities);
        }
        ...
        private static SheetData GenerateSheetdataForDetails<T>(List<T> data)
        {
            ...
            for (int i = 0; i < data.Count; i++)
            {
                ...
                {
                    var value = typeof(T).GetProperties()[column].GetValue(data[i]).ToString();
